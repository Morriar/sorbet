...
public:
    ...
    unique_ptr<Node> block(unique_ptr<Node> methodCall, const token *begin, unique_ptr<Node> args,
        ...
        bool numBlock = false;
        if (auto *numArgs = parser::cast_node<NumArgs>(args.get())) {
            numBlock = true;
            std::cout << "num\n";
        }
    ...
    unique_ptr<Node> numargs(unique_ptr<Node> params, size_t max_numparam) {
        return make_unique<NumArgs>(params->loc/* TODO , max_numparam*/);
    }
...
ForeignPtr numargs(SelfPtr builder, ForeignPtr params, size_t max_numparam) {
    auto build = cast_builder(builder);
    return build->toForeign(build->numargs(build->cast_node(params), max_numparam));
}
